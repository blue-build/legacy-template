...
> **Note**
> Everywhere in this repository, make sure to replace `bgibryan/ublueBG` with the details of your own repository. Unless you used [`create-ublue-image`](https://github.com/EinoHR/create-ublue-image), in which case the previous repo identifier should already be your repo's details.
...
Read [the README in the `scripts/` directory](https://github.com/bgibryan/ublueBG/blob/main/scripts/README.md) for more information.
...
- First rebase to the image unsigned, to get the proper signing keys and policies installed:
  ```
  sudo rpm-ostree rebase ostree-unverified-registry:ghcr.io/bgibryan/ublueBG:latest
  ```
...
- Then rebase to the signed image, like so:
  ```
  sudo rpm-ostree rebase ostree-image-signed:docker://ghcr.io/bgibryan/ublueBG:latest
  ```
...
```
sudo rpm-ostree rebase ostree-image-signed:docker://ghcr.io/bgibryan/ublueBG:20230403
```
...
To run the action, simply edit the `boot_menu.yml` by changing all the references to `bgibryan/ublueBG` to your repository. This should trigger the action automatically.
